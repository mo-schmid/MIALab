...
def collect_image_paths(data_dir):
    ...
    crawler = futil.FileSystemDataCrawler(data_dir, image_keys, MyFilePathGenerator(), dir_filter)
...
def load_images(image_paths):
    ...
    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(list(image_paths.values())[0], sitk.sitkFloat32),  # todo: modify here
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(list(image_paths.values())[1], sitk.sitkUInt8)  # todo: modify here
    }
...
def register_images(image_dict, atlas_img):
    ...
    registered_t1 = registration.execute(list(image_dict.values())[0], registration_params)# todo: modify here
...
def preprocess_filter_rescale_t1(image_dict, new_min_val, new_max_val):
    ...
    minmax_rescaled_img = filter.execute(list(image_dict.values())[0], filter_params)  # todo: modify here
...
def extract_feature_median_t1(image_dict):
    ...
    median_img = filter.execute(list(image_dict.values())[0])  # todo: modify here
