...
The result is evaluated on whether the improvements are statistically significant. The Dice index is evaluated using a t-test and the Hausdorff distance is tested using the Wilcoxon signed-rank. 
Because the change of the matrices before and after the post-processing is compared, the data sets are considered to be paired. For approximately normal distributed differences a paired t-test is used, here for the Dice Index. Otherwise a paired Wilcoxon signed-rank test, here for the Hausdorff distance. The normal distribution is checked graphically by QQ-plots. The data was considered approximately normal distributed if all data points lie within the $95\%$ confidence bounds. The corresponding QQ-Plots can be found in Appendix \ref{sec_qq} QQ-Plot.
...
\hline
\multicolumn{6}{p{270pt}}{This values are the result of the testing sample, $n=10$. }\\
\multicolumn{6}{p{270pt}}{$\mu=$ mean with no postprocessing; $\mu_{pkf}=$ mean with PKF postprocessing}\\
\multicolumn{6}{p{270pt}}{$\Delta=\mu_{pkf}-\mu$; p $=$ p-value;  $CI_{low}= 95\%$ confidence interval lower bound} \\
\multicolumn{6}{p{270pt}}{$CI_{high}= 95\%$ confidence interval upper bound.}
\end{tabular}
\end{table}
...
\hline
\multicolumn{6}{p{270pt}}{This values are the result of the testing sample, $n=10$. }\\
\multicolumn{6}{p{270pt}}{$m=$ median with no postprocessing; $m_{pkf}=$ median with PKF postprocessing}\\
\multicolumn{6}{p{270pt}}{$\Delta=m_{pkf}-m$; p $=$ p-value;  $CI_{low}= 95\%$ confidence interval lower bound} \\
\multicolumn{6}{p{270pt}}{$CI_{high}= 95\%$ confidence interval upper bound.}
\end{tabular}
\end{adjustwidth}
\end{table}
...
In figure \ref{fig_pkf} the visual improvement of the post-processing can be seen. Within the red circled area comes forward, that the miss labeled area is corrected. Additionally the figure demonstrates that with the PKF method the overall uniformity of the segments is increased and less holes (Background) or miss labeled voxels can be found.
...
\hline
\multicolumn{6}{p{270pt}}{This values are the result of the testing sample, $n=10$. }\\
\multicolumn{6}{p{270pt}}{$\mu=$ mean with no postprocessing; $\mu_{dcrf}=$ mean with DCRF postprocessing}\\
\multicolumn{6}{p{270pt}}{$\Delta=\mu_{dcrf}-\mu$; p $=$ p-value;  $CI_{low}= 95\%$ confidence interval lower bound} \\
\multicolumn{6}{p{270pt}}{$CI_{high}= 95\%$ confidence interval upper bound.}
\end{tabular}
\end{table}
...
\hline
\multicolumn{6}{p{270pt}}{This values are the result of the testing sample, $n=10$. }\\
\multicolumn{6}{p{270pt}}{$m=$ median with no postprocessing; $m_{dcrf}=$ median with PKF postprocessing}\\
\multicolumn{6}{p{270pt}}{$\Delta=m_{dcrf}-m$; p $=$ p-value;  $CI_{low}= 95\%$ confidence interval lower bound} \\
\multicolumn{6}{p{270pt}}{$CI_{high}= 95\%$ confidence interval upper bound.}
\end{tabular}
%\end{adjustwidth}
\end{table}
...
Figure \ref{fig_dcrf} shows the graphical difference of DCRF compared to the ground truth and no postprocessing. The improvement of the label Thalamus can be seen, since again the red parts to the left and right of the Thalamus regions are removed. These removed parts including some keyhole background regions are filled with the green label of white matter. 
...
Although the most differences between no postprocessing and postprocessing are significant their impact on the overall segmentation is questionable. For the PKF and the DCRF the largest changes in the metric was in both cases made in the Hausdorff distance. There we can see an improvement for the labels of the smaller brain regions (hippocampus, amygdala, thalamus). This Change can also be seen in the visual interpretation of the post processing, where the miss-labeled regions of the (Red label, thalamus) are corrected. The small changes in the dice coefficient could be explained by the fact, that both Methods do not alter the overall area of an label on a large extent. Thus correcting the labels of only a few voxels has almost no effect on the dice coefficient.
\section{Conclusion}
